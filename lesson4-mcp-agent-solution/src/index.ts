...
//   - MCPを使ってエージェントに外部ツールを追加する方法を学ぶ
//   - 「能力の後付け」という概念を体験する
//
// MCP とは？
//   - AIモデルと外部ツール/データを接続するための標準プロトコル
//   - MCPサーバーがツールを提供し、クライアントがそれを利用する
//   - これにより、コードを変更せずにエージェントの能力を拡張できる
//
// Lesson 3 との差分:
//   - @mastra/mcp パッケージを追加
//   - MCPClientでDeepWiki MCPに接続
//   - mcpClient.listTools()でツールをAgentに渡す
//   - GitHubリポジトリについて質問できるようになる
// ==========================================
...
// ========== メイン処理 ==========
async function main() {
  console.log(`${colors.mcp}[MCP]${colors.reset} DeepWiki MCPに接続中...`);

  // MCPからツール一覧を取得
  const mcpTools = await mcpClient.listTools();
  console.log(
    `${colors.mcp}[MCP]${colors.reset} 接続完了！利用可能なツール:`,
        ...
        onStepFinish: (step) => {
          ...
          if (step.toolCalls && step.toolCalls.length > 0) {
            for (const toolCall of step.toolCalls) {
              ...
              const isMcpTool = toolName && toolName in mcpTools;
              const color = isMcpTool ? colors.mcp : colors.tool;
              const label = isMcpTool ? "[MCP]" : "[Tool]";
              console.log(`  ${color}${label} Toolを呼び出します →${colors.reset}`, toolName);
            }
          }
          // Tool実行結果を表示
          if (step.toolResults && step.toolResults.length > 0) {
            for (const toolResult of step.toolResults) {
              ...
              const isMcpTool = toolName && toolName in mcpTools;
              const color = isMcpTool ? colors.mcp : colors.tool;
              const label = isMcpTool ? "[MCP]" : "[App]";
              console.log(`  ${color}${label} 実行結果 →${colors.reset}`, resultStr);
            }
          }
        },
      });
